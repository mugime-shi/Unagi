...
export function ZoneComparison() {
  ...
  if (loading) return <p className="text-gray-500 text-sm">Loading zone comparison…</p>
  if (error)   return <p className="text-red-400 text-sm">Failed to load: {error.message}</p>
  ...
  if (points.length === 0) {
    return (
      <div className="bg-gray-900 rounded-2xl p-4 space-y-2">
        <h2 className="text-sm font-medium text-gray-300">Zone Comparison — SE1–SE4</h2>
        <p className="text-gray-500 text-sm">
          No multi-zone data yet. Run a backfill for SE1, SE2, SE4 to populate the chart.
        </p>
  ...
  return (
    <div className="bg-gray-900 rounded-2xl p-4 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-2 text-center">
        {summaries.map(({ area, city, avg }) => (
          <div key={area} className="bg-gray-800 rounded-xl py-3">
            <p className="text-xs mb-0.5" style={{ color: ZONE_COLORS[area] }}>{area}</p>
            <p className="text-xs text-gray-500 mb-1">{city}</p>
            <p className="text-sm font-semibold">{avg != null ? avg.toFixed(3) : '—'}</p>
      ...
      <p className="text-xs text-gray-700 text-center">
        SE1 (north) is typically cheapest · SE4 (south) most expensive · gap reflects transmission constraints
      </p>
    </div>
  )
}
