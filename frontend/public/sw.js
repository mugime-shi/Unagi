/**
 * Namazu Service Worker — handles Web Push notifications.
 *
...
self.addEventListener('push', (event) => {
  let data = { title: 'Namazu', body: 'Tomorrow\'s prices are ready' }
  try {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title, {
      ...
      icon: '/icon.svg',
      badge: '/icon.svg',
      tag: 'namazu-price',    // replaces previous notification so you don't get spammed
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((windows) => {
      const existing = windows.find((w) => new URL(w.url).pathname === target)
