...
export function ForecastAccuracy({ area }) {
  ...
  if (breakdownBy && breakdown?.models) {
    ...
    chartData = [...allKeys].sort((a, b) => a - b).map((key) => {
      const label = breakdownBy === 'weekday' ? WEEKDAY_LABELS[key] : `${String(key).padStart(2, '0')}:00`
      const row = { label }
      ...
      {chartData && chartData.length > 0 && (
        <div className="mt-4">
          <p className="text-xs text-gray-500 mb-2">
            MAE by {breakdownBy === 'weekday' ? 'day of week' : 'hour of day'} (SEK/kWh)
          </p>
