...
### 2.2-B Nord Pool Day-Ahead 入札タイムラインとデータ可用性

Day D の価格を予測・取得するための時系列（全て CET）:

```
Day D-1（前日）
  10:00  TSO が送電容量（NTC/フローベースパラメータ）を公開
  12:00  入札締切（Gate Closure）— 全ての売買注文を確定
 ~12:45  価格公表目標（Euphemia アルゴリズム結果）
         ※通常 12:45〜13:00、遅延時は最大 13:45 頃
 ~13:00  ENTSO-E Transparency Platform に反映（通常 30〜60分後）
  13:30  Namazu Daily Fetch ← ここで Day D のスポット価格を取得
  15:00  イントラデイ IDA1 締切

Day D-1 → Day D（夜間）
  22:00  イントラデイ IDA2 締切
  01:05  Namazu Nightly Predict ← 前日の発電/インバランス確定後に予測

Day D（当日）
  10:00  イントラデイ IDA3 締切
  各時間  連続取引（SIDC）: 配送60分前まで取引可能
```

**Namazu にとっての重要ポイント:**

1. **予測の最大価値は入札締切（12:00 CET）前**。01:05 CET の Nightly Predict は約11時間前に予測を提供
2. **13:30 CET の Daily Fetch は通常ケースに最適**だが、Euphemia 遅延（~13:45）でミスする可能性あり。リトライロジックで対応
3. **前日の発電ミックス・インバランスデータ**は 1-2 時間遅延で断続的に発表され、~01:00 CET に全24時間が確定。Nightly cron はこのタイミング後に実行
