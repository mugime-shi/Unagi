...
### 6.1 CET/CEST 夏時間対応
- [ ] バックエンド全箇所の `UTC+1` 固定を `ZoneInfo("Europe/Stockholm")` に置き換え
...
- [ ] バックエンド: `GET /api/v1/prices/today?area=SE1` など既存の area パラメータを全エンドポイントで公開
  - `prices.py` のエンドポイントに `area: str = Query("SE3", ...)` を追加
  - `history`, `range`, `cheapest-hours` も対応
- [ ] フロントエンド: ヘッダーにエリアセレクター（SE1/SE2/SE3/SE4）を追加
  - 選択したエリアを全 API リクエストに渡す
  - ヘッダーの `SE3 · Göteborg` をエリアに応じて動的に変更
- **なぜ**: バックエンドは準備済みなのにUIがSE3固定は機会損失。スウェーデン全土対応になる

### 6.3 マルチゾーン比較チャート
- [ ] `GET /api/v1/prices/multi-zone?date=YYYY-MM-DD` 新エンドポイント
  - SE1-SE4 の同日価格を並行取得してまとめて返す
- [ ] History ページに「Zone Comparison」タブを追加
  - 4ゾーンの1時間粒度価格を同一チャートに重ねる（Recharts MultiLine）
  - ゾーン間の価格差（送電ボトルネック）を可視化
- **なぜ**: 北欧電力市場の構造（北部安 / 南部高）をビジュアルで証明できる。面接での差別化

### 6.4 明日の価格プッシュ通知（PWA）
- [ ] `frontend/public/sw.js`: Service Worker（プッシュ通知受信）
- [ ] `frontend/src/hooks/usePushNotification.js`: 通知権限取得 + 購読
- [ ] バックエンド: `POST /api/v1/notify/subscribe` でサブスクリプション保存
- [ ] バックエンド: EventBridge 13:30 CET トリガー → 翌日の min/max 時間帯を通知
- **なぜ**: 「毎日使う」を本当に実現。PROJECT_BRIEF の約束を果たす

### 6.5 簡易価格予測
- [ ] `GET /api/v1/prices/forecast?date=tomorrow` エンドポイント
  - 過去30日の同曜日・同時間帯の平均価格で翌日レンジを予測
  - SMHI 天候予報 API があれば係数として組み込む
- [ ] フロントエンド: tomorrow タブに予測バンド（薄色の range area）を表示
