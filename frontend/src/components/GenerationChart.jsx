  ...
  return (
    <div className="bg-gray-900 rounded-2xl p-4">
      <div className="flex items-start justify-between mb-3">
        <div>
          <h2 className="text-sm font-medium text-gray-300">Generation mix · MW</h2>
          {(renewable_pct != null || carbon_free_pct != null) && (
            <p className="text-xs text-gray-500 mt-0.5">
              {renewable_pct != null && <span className="text-green-400">Renewable {renewable_pct}%</span>}
              {renewable_pct != null && carbon_free_pct != null && <span className="text-gray-600"> · </span>}
              {carbon_free_pct != null && <span>Carbon-free {carbon_free_pct}%</span>}
              {latest_slot && <span className="ml-2">· {lagLabel(latest_slot)}</span>}
            </p>
          )}
        </div>
