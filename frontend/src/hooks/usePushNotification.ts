...
import type { Area, NotificationStatus } from "../types/index";
...
export function usePushNotification(area: Area): UsePushNotificationReturn {
  ...
  async function checkStatus(): Promise<NotificationStatus> {
  ...
  async function subscribe(): Promise<void> {
    ...
    try {
      ...
      const { public_key }: { public_key: string } = await keyRes.json();
